...
export function App() {
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-white text-gray-900 font-sans antialiased selection:bg-emerald-100 selection:text-emerald-900">
      {/* Mobile Backdrop */}
      {sidebarOpen && (
        <div
      ...
      {/* ChatGPT Contrast Sidebar (Dark Charcoal Black) */}
      <aside
        className={`fixed md:static inset-y-0 left-0 z-40 bg-[#171717] border-r border-[#262626] flex flex-col transition-all duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="p-3 border-b border-[#2a2a2a]">
          <div className="flex items-center justify-between px-2 py-1 mb-3">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h1 className="text-xs font-bold text-white tracking-tight">The Lenny Assistant</h1>
                <p className="text-[10px] text-emerald-400 font-mono font-medium">FDE Take-Home Demo</p>
              </div>
            </div>
            <button
              onClick={() => setSidebarOpen(false)}
          ...
          <button
            ...
            className="w-full py-2.5 px-3.5 rounded-xl bg-[#212121] hover:bg-[#2a2a2a] text-white font-semibold text-xs flex items-center justify-between border border-[#333333] shadow-sm hover:border-[#444444] transition-all group"
          >
            <div className="flex items-center gap-2">
              <Plus className="w-4 h-4 text-emerald-400 group-hover:scale-110 transition-transform" />
              <span>New Thread</span>
            </div>
            <span className="text-[10px] font-mono text-gray-400 bg-[#171717] px-1.5 py-0.5 rounded border border-[#333333]">
              ⌘K
            </span>
          </button>
        </div>
        ...
        <div className="flex-grow overflow-y-auto px-2 py-3 space-y-1">
          ...
          {sessions.length === 0 ? (
            <div className="text-xs text-gray-400 p-3 italic text-center">No previous threads</div>
          ) : (
            sessions.map((s) => (
              <div
                key={s.id}
                ...
                className={`group flex items-center justify-between px-3 py-2.5 rounded-xl text-xs cursor-pointer transition-all ${
                  activeSessionId === s.id
                    ? 'bg-[#2a2a2a] text-white font-semibold shadow-sm border border-[#383838]'
                    : 'text-gray-300 hover:bg-[#212121] hover:text-white'
                }`}
              >
                <div className="flex items-center gap-2.5 truncate">
                  <MessageSquare className={`w-3.5 h-3.5 flex-shrink-0 ${activeSessionId === s.id ? 'text-emerald-400' : 'text-gray-400'}`} />
                  <span className="truncate">{s.title || 'Growth Session'}</span>
                </div>
                <button
                  onClick={(e) => handleDeleteSession(e, s.id)}
                  className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-[#333333] text-gray-400 hover:text-rose-400 transition-all"
        ...
        <div className="p-3 border-t border-[#262626] bg-[#0f0f0f] text-[11px] space-y-2">
          ...
          <div className="pt-1.5 border-t border-[#262626] flex items-center justify-between text-[10px] text-gray-400">
      ...
      {/* Main Area: White Chat Pane + Claude Artifact Canvas */}
      <main className="flex-grow flex flex-col md:flex-row min-w-0 h-full overflow-hidden bg-white">
        {/* Center Chat View */}
        <div
        ...
        {activeArtifact && (
          <div className="w-full md:w-1/2 lg:w-2/5 h-full z-20 border-l border-gray-300 shadow-2xl animate-in slide-in-from-right duration-300">
            <ArtifactViewer
